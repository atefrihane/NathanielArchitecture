  ...
  return text.toString().toLowerCase()
	...
	$(document).ready(function() {
		...
		$('#side-menu').mouseover(function() {
			$('#projects-menu').children('img').css('visibility', 'visible');
			$('#about-menu').children('img').css('visibility', 'visible');
			$('#admin-menu').children('img').css('visibility', 'visible');
			$('#project-info').css('visibility', 'visible');
			$('#project-info').children('ul').css('visibility', 'visible');
			$('#page-controls').css('visibility', 'visible');
		});

		$('#side-menu').mouseout(function() {
			$('#projects-menu').children('img').css('visibility', 'hidden');
			$('#about-menu').children('img').css('visibility', 'hidden');
			$('#admin-menu').children('img').css('visibility', 'hidden');
			$('#project-info').css('visibility', 'hidden');
			$('#project-info').children('ul').css('visibility', 'hidden');
		...
		if(data.length > 0) {
			$('#project-info-name').html(data[0].project.name);
			$('#project-info-architect').html(data[0].project.architect);
			$('#project-info-location').html(data[0].project.location);
			$('#farchitect').attr('href', '/?section=projects&sarchitect='+slugify(data[0].project.architect));
			$('#flocation').attr('href', '/?section=projects&slocation='+slugify(data[0].project.location));
		}
	...
	function slide(id, showTime, fadeTime) {
		...
		function slideMain(index) {
			...
			if($(".slider-page").eq(cur).children("img").attr("src")=="/Icons/loading_big.gif"){
				...
				function fii() {
					...
					$("#project-info-name").html(data[next].project.name);
					$("#project-info-architect").html(data[next].project.architect);
					$("#project-info-location").html(data[next].project.location);
